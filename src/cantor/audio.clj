(ns cantor.audio
  (:require [overtone.core :refer [boot-external-server kill-server server-connected? ctl kill midi->hz volume]]
            [cantor.synthdefs :refer [cantor-note]]))
...
(defn open!
  ...
  (when-not (server-connected?)
    ...
    ; Calling cantor-note triggers load-synthdef which uses with-server-sync —
    ; flushes all pending /s_new for mixer nodes before volume is set.
    (kill (cantor-note :amp 0))
    (volume 1))
...
(defn note-on
  ...
  [pitch velocity]
  (let [node (cantor-note :freq (midi->hz pitch)
